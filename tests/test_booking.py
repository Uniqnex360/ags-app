...
async def test_t15_happy_path_booking(session, session_factory):
    ...
    async with AsyncClient(
        ...
        assert body["ref_code"].startswith("AGS-")
...
async def test_t16_concurrency_race_same_seat(session, session_factory):
    ...
    user_a = User(id=uuid.uuid4(), email="userA@ags.local", password_hash="h")
    user_b = User(id=uuid.uuid4(), email="userB@ags.local", password_hash="h")
    user_c = User(id=uuid.uuid4(), email="userC@ags.local", password_hash="h")
...
async def test_t17_idempotency_key(session, session_factory):
    ...
    user = User(id=uuid.uuid4(), email="idem@ags.local", password_hash="h")
    session.add(user)
    await session.commit()
...
async def test_t18_cancel_frees_seats(session, session_factory):
    ...
    user1 = User(id=uuid.uuid4(), email="u1@ags.local", password_hash="h")
    user2 = User(id=uuid.uuid4(), email="u2@ags.local", password_hash="h")
