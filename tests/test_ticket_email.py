...
async def _seed_data(session):
    cinema = Cinema(
        id=uuid.uuid4(), name="AGS Lulu Mall", city="Kochi", timezone="Asia/Kolkata"
    )
...
async def test_t19_email_sent_and_public_ticket_link(session, session_factory):
    test_email_service = ConsoleEmailService(web_base_url="http://ags.demo")
    ...
    user = User(id=uuid.uuid4(), email="ticket_fan@ags.local", password_hash="h")
    session.add(user)
    await session.commit()
    ...
    async with AsyncClient(
        ...
        assert sent["to"] == "ticket_fan@ags.local"
