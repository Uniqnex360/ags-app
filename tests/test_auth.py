...
async def test_t9_register_login_me(session):
    ...
    async with AsyncClient(
        ...
        reg_resp = await client.post(
            "/v1/auth/register",
            json={"email": "alice@ags.local", "password": "password123"},
        )
        ...
        assert reg_data["user"]["email"] == "alice@ags.local"
        ...
        assert me_data["email"] == "alice@ags.local"
...
async def test_t10_duplicate_register_conflict(session):
    ...
    async with AsyncClient(
        ...
        resp1 = await client.post(
            "/v1/auth/register",
            json={"email": "bob@ags.local", "password": "password123"},
        )
        assert resp1.status_code == 201

        resp2 = await client.post(
            "/v1/auth/register",
            json={"email": "bob@ags.local", "password": "password456"},
        )
...
async def test_t11_invalid_tokens_unauthorized(session):
    ...
    async with AsyncClient(
        ...
        resp_wrong_login = await client.post(
            "/v1/auth/login",
            json={"email": "nonexistent@ags.local", "password": "pass"},
        )
